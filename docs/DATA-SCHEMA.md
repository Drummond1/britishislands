...
|---|---|---|---|
| `id` | string | yes | Stable, lowercase-hyphenated, ASCII-folded name. Disambiguated by parent body or island group on collision. **Never regenerated** for an existing island. Used for `?island=` deep links. |
| `seoPath` | string | no (stamped on shards + index) | Public canonical path `/islands/{nation}/{slug}/` from `scripts/seo_paths.py`. Compact index key: `sp`. Not hand-edited; regenerated by `build_islands_index.py`. Atlas address bar and share links use this path. |
